...
//       instance.where(or(eq(table.email, account), eq(table.mobile, account))),
//     );
...
//     const { email } = await users.queryById(id);
...
//     const res = await users.update(id, {
...
//   const info = await users.queryById(res.id);
